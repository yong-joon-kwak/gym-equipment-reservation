...
flowchart TD
    %% 노드
    UC_Settle["M 이 E 를 태깅<br/>→ 지연 정리: E 와 M 의 만료 세션 종료 기록 · 노쇼 처리 · 호출 확정"]
...
- 대기 등록이 곧 만료의 시작이다. 대기자가 없는 사용 세션에는 만료 시각이 없고, 첫 대기가 생길 때 정해진다(루트 README §3).
- **관리자는 이 판정을 타지 않는다.** 관리자가 QR 을 찍으면 그 기구의 관리 화면(현황 · 강제 종료 · 비활성화)으로 간다.
