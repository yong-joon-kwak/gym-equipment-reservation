...
- **판정은 백엔드가 한다.** 프론트는 `422 equipment_inactive` / `422 requeue_blocked` 같은 사유([`backend.md` §3](backend.md))를 받아 **그대로 노출**하고, 자체 판정을 두지 않는다.
...
- 로그인 판정처럼 "상태를 프론트가 해석"해야 하는 것도, **로컬 값이 아니라 서버 응답을 최종 기준**으로 삼는다(세션 쿠키 로그인 — [`backend.md` §5.2](backend.md)).
