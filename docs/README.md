...
기능 문서는 **첫 T3 가 생길 때 `in-progress` 로 바꾼다.** 도메인 규칙(T1)만 구현된 동안은 `planned` 로 둔다 — `in-progress` 는 커버리지 하한 가드의 대상이라, T3 없이 바꾸면 가드가 실패한다. 규칙만 있는 상태는 아직 흐름이 도는 기능이 아니다([`plans/2-3-feature-guard.md`](plans/2-3-feature-guard.md) D5).
